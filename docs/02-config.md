...
The host clone lives at `$AGENTIC_HOME/marketplaces/<slug>-<hash>/`, where `<hash>` is a short hash of `url` and `<slug>` is a human-readable label taken from `url` itself (not from `name`). The clone directory is a pure function of `url`, so it's shared by every project on the machine that references that `url` - regardless of what `name` each project happens to give it - and two different `url`s never collide. The container-side mount path stays `name`-only, since that's scoped to one project's sync and duplicate names within a single project are already rejected.
...
Each successful sync records the current working directory in a small usage registry (`$AGENTIC_HOME/marketplaces/.usage.json`), keyed by both the clone and the local `name` a project gives it - so more than one project can safely share the same clone, including under different names. Use `agentic marketplaces list` to see every synced clone and which project(s) reference it under which name(s) - a clone referenced under two different names shows as two rows sharing the same URL - and `agentic marketplaces prune` to remove clones no known project references anymore under any name. Prune re-checks every recorded project's _current_ config before deleting anything: a clone directory is only deleted from disk once none of its recorded names have a live project left; if some names are still live and others aren't, only the dead names are dropped from the registry and the shared clone directory is kept. A clone with no usage record at all (e.g. placed there manually) is left alone rather than guessed about. `agentic clean` does not touch marketplace clones.
