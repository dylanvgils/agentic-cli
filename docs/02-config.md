...
Before each `agentic run`/`agentic <tool>`, every applicable marketplace is synced on the host - `git clone` if new, or `git fetch` + `git reset --hard @{upstream}` if already cloned - using the invoking user's own git auth (SSH agent, credential helper, `~/.netrc`); no credentials enter the container. Requires `git` on the host `PATH`.
...
The clone is bind-mounted read-only into the container, so the container never runs git or reaches a git host over the network. Marketplaces are shared across tools too: one with no `tools` filter clones once on the host and mounts into every supporting tool that runs, rather than cloning separately per tool.

The host clone lives at `$AGENTIC_HOME/marketplaces/<slug>-<hash>/`, where `<hash>` is a short hash of `url` and `<slug>` a human-readable label taken from `url` (not `name`). Since the directory is a pure function of `url`, it's shared by every project on the machine that references that `url` regardless of `name`, and different `url`s never collide. The container-side mount path stays `name`-only, scoped to one project's sync (duplicate names within a project are already rejected).

Every tool mounts the clone at the same neutral, agentic-owned container path, `~/marketplaces/<name>`, rather than nesting it inside the tool's own config tree - so it can never collide with marketplace state a tool persists for itself.

Mounting the clone alone isn't enough: both tools only recognize marketplaces registered in their own config, not whatever happens to be present on disk. Each tool's `entrypoint.sh` handles this - before `exec`-ing the tool, it registers every name listed in `AGENTIC_MARKETPLACES` (set by `agentic run` to this run's configured marketplaces only), skipping names with no matching mounted directory:

- `claude` runs `claude plugin marketplace add <dir> --scope user`, which is idempotent and needs no network access for an already-mounted local directory. The registered name comes from the marketplace's own `.claude-plugin/marketplace.json`, which may differ from the `name` you gave it in `.agenticrc.toml` (that name only controls the mount path).
...
Neither script globs `~/marketplaces` directly - it only registers the names listed in `AGENTIC_MARKETPLACES` for this run.

Each successful sync records the current working directory in a usage registry (`$AGENTIC_HOME/marketplaces/.usage.json`), keyed by both the clone and the local `name` a project gives it, so multiple projects can safely share a clone under different names. `agentic marketplaces list` shows every synced clone and which project(s) reference it under which name(s) - a clone referenced under two different names shows as two rows sharing the same URL. `agentic marketplaces prune` removes clones no known project references anymore under any name; it re-checks every recorded project's _current_ config first, so a clone directory is only deleted once none of its recorded names have a live project left - if some names are still live and others aren't, only the dead names are dropped from the registry and the shared clone directory is kept. A clone with no usage record at all (e.g. placed there manually) is left alone rather than guessed about. `agentic clean` does not touch marketplace clones.
