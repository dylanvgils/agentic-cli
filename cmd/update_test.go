...
func stubUpdateTool(t *testing.T, fn func(tool, image, versionCmd string, opts docker.BuildOptions) error) func() {
	t.Helper()
	orig := updateTool
	updateTool = fn
...
func TestDryRunUpdate_printsDockerfile_skipsScript(t *testing.T) {
	...
	restore := stubUpdateTool(t, func(_, _, _ string, _ docker.BuildOptions) error {
		scriptCalled = true
		return nil
	})
	defer restore()
	...
	out := captureStdout(t, func() {
		err := dryRunUpdate([]string{"claude"}, docker.BuildOptions{Versions: map[string]string{}})
...
func TestDryRunUpdate_withoutToolArg_returnsError(t *testing.T) {
	// Act
	err := dryRunUpdate([]string{}, docker.BuildOptions{Versions: map[string]string{}})
...
func TestUpdateTools_allBuilt_updatesAll(t *testing.T) {
	// Arrange
	var updated []string
	restore := stubUpdateTool(t, func(tool, _, _ string, _ docker.BuildOptions) error {
	...
	// Act
	err := updateTools([]string{}, docker.BuildOptions{Versions: map[string]string{}})
...
func TestUpdateTools_singleTool_alwaysUpdates(t *testing.T) {
	...
	// Act
	err := updateTools([]string{"claude"}, docker.BuildOptions{Versions: map[string]string{}})
...
func TestUpdateOneTool_versionChanged_reported(t *testing.T) {
	// Arrange
	restore := stubUpdateTool(t, func(_, _, _ string, _ docker.BuildOptions) error { return nil })
	defer restore()
	...
	// Act
	out := captureStdout(t, func() {
		err := updateOneTool("claude", docker.BuildOptions{Versions: map[string]string{}})
		require.NoError(t, err)
	})
...
func TestRunUpdate_baseFlag_setsOpt(t *testing.T) {
	...
	restore := stubUpdateTool(t, func(_, _, _ string, opts docker.BuildOptions) error {
		capturedOpts = opts
		return nil
	})
	defer restore()
