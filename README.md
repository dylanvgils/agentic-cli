...
| `clean [tool] [--namespace <name>] [--all] [--home <dir>]`                                                                                                                                                          | Remove tool image(s). `--all` removes across all namespaces. No-arg form also removes base images, the proxy image, leftover proxy resources, the `agentic-net` network, and any on-disk marketplace clone no longer referenced by config |
...
`$AGENTIC_HOME/marketplaces/<name>/` holds host-side clones of any `[[marketplaces]]` configured in `.agenticrc.toml` (see [docs/02-config.md](docs/02-config.md)) - shared across tools and mounted read-only into each applicable tool's container, never written to by the container itself.
