...
The proxy image is built on demand the first time you run with `--proxy` if it is missing. A released agentic installs its matching version from the published module; a dev build (version `dev`) compiles the proxy from the local source tree, so run `agentic run --proxy` from within the agentic repository. `build` and `update` themselves are not proxied - they need broad network access for apt and package installs.
