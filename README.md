...
| `clean [tool] [--namespace <name>] [--all] [--home <dir>]`                                                                                                                                                          | Remove tool image(s). `--all` removes across all namespaces. No-arg form also removes base images, the proxy image, leftover proxy resources, and the `agentic-net` network                  |
...
`$AGENTIC_HOME/marketplaces/<name>-<hash>/` holds host-side clones of any `[[marketplaces]]` configured in `.agenticrc.toml` (see [docs/02-config.md](docs/02-config.md)) - shared across tools and mounted read-only into each applicable tool's container, never written to by the container itself. The directory name is suffixed with a short hash of the marketplace's `url` so two projects that happen to pick the same `name` for different URLs never collide on the same clone.
...
```bash
agentic marketplaces list    # List synced clones and which project(s) reference each (alias: ls)
agentic marketplaces prune   # Remove clones no project references anymore
```

Because the clone store under `$AGENTIC_HOME/marketplaces/` is shared across every project on the machine, `prune` only removes a clone once none of the projects that have ever synced it (tracked in a small usage registry alongside the clones) still reference it - it never assumes the project you're pruning from is the only one that matters. A clone with no usage record (e.g. placed there manually) is left alone and reported instead of deleted.
