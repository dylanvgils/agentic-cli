...
| `clean [tool] [--namespace <name>] [--all]`                                                                                                                                                                                                                                   | Remove tool image(s). `--all` removes across all namespaces. No-arg form also removes base images, the proxy image, leftover proxy resources, all filesystem audit logs under `$AGENTIC_HOME/logs/`, and the `agentic-net` network                       |
...
| `migrate [--home <dir>]`                                                                                                                                                                                                                                                      | Apply any pending migrations to `$AGENTIC_HOME`'s on-disk layout. Runs automatically before most commands; use this to trigger it explicitly                                                                                                            |
...
| `run [flags] <tool> [args...]`                                                                                                                                                                                                                                                | Run a tool in an isolated Docker container. `--proxy` / `--no-proxy` toggle the egress allowlist proxy for the run; `--proxy-monitor` runs it without blocking, just logging hosts contacted. `--audit` / `--no-audit` toggle filesystem audit logging   |
